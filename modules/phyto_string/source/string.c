...
static int32_t phyto_string_compare_internal(phyto_string_t a, phyto_string_t b) {
...
phyto_string_t phyto_string_center(phyto_string_span_t span, size_t width, char fill) {
    phyto_string_t result = phyto_string_new();
    if (span.size >= width) {
...
size_t phyto_string_count_sub(phyto_string_span_t span, phyto_string_span_t sub) {
...
bool phyto_string_starts_with(phyto_string_span_t span, phyto_string_span_t sub) {
    if (span.size < sub.size) {
        return false;
    }
    if (sub.size == 0) {
...
phyto_string_t phyto_string_expand_tabs(phyto_string_span_t span, size_t tab_width) {
    if (tab_width == 0) {
...
bool phyto_string_find(phyto_string_span_t span, phyto_string_span_t sub, size_t* out_index) {
...
phyto_string_t phyto_string_join(phyto_string_span_t sep, phyto_string_vec_t args) {
    phyto_string_t result = phyto_string_new();
...
phyto_string_t phyto_string_left_justify(phyto_string_span_t span, size_t width, char fill) {
    phyto_string_t result = phyto_string_new();
...
phyto_string_vec_t phyto_string_partition(phyto_string_span_t span, phyto_string_span_t sep) {
    phyto_string_vec_t result = phyto_string_vec_init(&phyto_string_vec_callbacks);
    size_t sep_pos;
    ...
    phyto_string_vec_append(&result, phyto_string_own(phyto_string_span_subspan(span, 0, sep_pos)));
    phyto_string_vec_append(&result, phyto_string_own(sep));
    phyto_string_vec_append(
        &result, phyto_string_own(phyto_string_span_subspan(span, sep_pos + sep.size, span.size)));
    return result;
}

phyto_string_t phyto_string_remove_prefix(phyto_string_span_t span, phyto_string_span_t prefix) {
    if (!phyto_string_starts_with(span, prefix)) {
        return phyto_string_own(span);
    }
    return phyto_string_own(phyto_string_span_subspan(span, prefix.size, span.size));
}

phyto_string_t phyto_string_remove_suffix(phyto_string_span_t span, phyto_string_span_t suffix) {
    if (!phyto_string_ends_with(span, suffix)) {
        return phyto_string_own(span);
    }
    return phyto_string_own(phyto_string_span_subspan(span, 0, span.size - suffix.size));
}
...
phyto_string_t phyto_string_replace(phyto_string_span_t span,
    ...
    while (true) {
        ...
        if (!phyto_string_find_in_range(span, old, pos, span.size, &next_pos)) {
            phyto_string_extend(&result, phyto_string_span_subspan(span, pos, span.size));
            return result;
        }
        phyto_string_extend(&result, phyto_string_span_subspan(span, pos, next_pos));
        phyto_string_extend(&result, new);
        pos = next_pos + old.size;
    }
}

bool phyto_string_find_last(phyto_string_span_t span, phyto_string_span_t sub, size_t* out_index) {
...
bool phyto_string_find_last_in_range(phyto_string_span_t span,
    ...
    while (true) {
        ...
        index -= sub.size;
        if (phyto_string_span_equal(span, phyto_string_span_subspan(span, index, sub.size))) {
            *out_index = index;
            return true;
        }
    }
...
phyto_string_t phyto_string_right_justify(phyto_string_span_t span, size_t width, char fill) {
    phyto_string_t result = phyto_string_new();
    size_t size = span.size;
    if (size >= width) {
        return phyto_string_own(span);
...
phyto_string_vec_t phyto_string_partition_last(phyto_string_span_t span, phyto_string_span_t sep) {
    phyto_string_vec_t result = phyto_string_vec_init(&phyto_string_vec_callbacks);
    size_t sep_pos;
...
phyto_string_vec_t phyto_string_split(phyto_string_span_t span, phyto_string_span_t sep) {
    phyto_string_vec_t result = phyto_string_vec_init(&phyto_string_vec_callbacks);
    size_t sep_pos;
    while (true) {
        if (!phyto_string_find(span, sep, &sep_pos)) {
        ...
        phyto_string_vec_append(&result,
                                phyto_string_own(phyto_string_span_subspan(span, 0, sep_pos)));
...
phyto_string_vec_t phyto_string_split_reverse(phyto_string_span_t span, phyto_string_span_t sep) {
    phyto_string_vec_t result = phyto_string_vec_init(&phyto_string_vec_callbacks);
    size_t sep_pos;
    while (true) {
        if (!phyto_string_find_last(span, sep, &sep_pos)) {
...
phyto_string_vec_t phyto_string_split_whitespace(phyto_string_span_t span) {
    ...
    while (true) {
        ...
        phyto_string_vec_append(
            &result, phyto_string_own(phyto_string_span_subspan(span, 0, sep.begin - span.begin)));
...
phyto_string_vec_t phyto_string_split_lines(phyto_string_span_t span) {
    ...
    while (true) {
        ...
        phyto_string_vec_append(&result,
                                phyto_string_own(phyto_string_span_subspan(span, 0, newline_pos)));
...
phyto_string_t phyto_string_strip_chars(phyto_string_span_t span, phyto_string_span_t chars) {
    size_t i;
    for (i = 0; i < span.size; i++) {
