...
LOX_EXPR_VISITOR_IMPL(lox_ast_printer, phyto_string_t);

phyto_string_t lox_print_ast(lox_expr_t* expr) {
    lox_ast_printer_t printer = {0};
    return lox_expr_accept_lox_ast_printer(expr, &printer);
}

LOX_EXPR_VISITOR_VISIT_BINARY_FUNC(lox_ast_printer, phyto_string_t) {
    phyto_string_t left = lox_expr_accept_lox_ast_printer(node->left, visitor);
    phyto_string_t right = lox_expr_accept_lox_ast_printer(node->right, visitor);
...
LOX_EXPR_VISITOR_VISIT_GROUPING_FUNC(lox_ast_printer, phyto_string_t) {
    phyto_string_t expr = lox_expr_accept_lox_ast_printer(node->expression, visitor);
    phyto_string_t result = phyto_string_new();
...
LOX_EXPR_VISITOR_VISIT_LITERAL_FUNC(lox_ast_printer, phyto_string_t) {
    (void)visitor;
    return lox_object_to_string(node->value);
}

LOX_EXPR_VISITOR_VISIT_UNARY_FUNC(lox_ast_printer, phyto_string_t) {
    phyto_string_t expr = lox_expr_accept_lox_ast_printer(node->right, visitor);
    phyto_string_t result = phyto_string_new();
